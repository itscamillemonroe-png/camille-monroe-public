...
async function loginPage(){
  ...
  $('#signupForm')?.addEventListener('submit',async(e)=>{
    ...
    try{
      ...
      const age_confirmed=$('#ageConfirmed').checked;
      const file=$('#profilePhoto').files[0];
      if(!age_confirmed)throw new Error('You must confirm you are 18 or older to request access.');
      ...
      const {data,error}=await supabase.auth.signUp({email,password,options:{data:{full_name,age_confirmed},emailRedirectTo:'https://itscamillemonroe.art/login/'}});
...
async function loadGallery(){
  ...
  gallery.innerHTML=items.map(item=>`<article class="mediaCard">${mediaElement(item)}<div><strong>${escapeHtml(item.title)}</strong><small>${item.audience_scope==='adult_content'?'Age-verified release':'Member release'}</small></div></article>`).join('');
}
...
async function memberPage(){
  ...
  const [profileR,subR,walletR]=await Promise.all([
    supabase.from('member_profiles').select('full_name,email,status,is_admin,profile_photo_path,approved_content_scope').eq('user_id',user.id).maybeSingle(),
  ...
  else if(approved&&active){banner.innerHTML='<strong>Member access active</strong><span>Your approved SFW member experience is open. Adult releases remain separately age-gated.</span>';banner.classList.add('activeAccess');}
  ...
  const {data:posts,error:postsError}=await supabase.from('member_posts').select('id,title,body,visibility_scope,published_at').order('published_at',{ascending:false}).limit(30);
