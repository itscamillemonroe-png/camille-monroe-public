...
async function init(){
  ...
  if(profile.is_admin)$('#callRequestForm').hidden=true;else if(profile.status!=='approved'||profile.verification_status!=='verified'){$('#callRequestForm').hidden=true;setCallStatus('Paid voice and video calls are part of the adult service lane and remain locked until a compliant 18+ check is active.','error');}
